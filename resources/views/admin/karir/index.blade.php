...
@section('content')
    <style>
        ...
        @media (max-width: 768px) {
            ...
            /* Kolom deskripsi tetap ditampilkan dengan max-width */
            .deskripsi-cell {
    ...
    <div class="flex justify-between items-center mb-4 header-action-row">
        <h2 class="text-[#2E6C9F] font-bold text-xl uppercase tracking-wider">Manajemen Karir</h2>
        <a href="{{ route('admin.karir.create') }}"
            ...
            <i class="fas fa-plus mr-2"></i> Tambah Karir
        </a>
    </div>

    {{-- Search Bar --}}
    <div class="mb-4 search-wrapper flex items-center gap-2">
        <form method="GET" action="{{ route('admin.karir.index') }}" id="searchForm" class="w-full max-w-sm">
            <div class="relative">
                <span class="absolute inset-y-0 left-3 flex items-center text-gray-400 pointer-events-none">
                    <i class="fas fa-search text-xs"></i>
                </span>
                <input type="text" name="search" id="searchInput" placeholder="Cari nama karir..."
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="text-[#2E6C9F] text-sm font-bold border-b border-gray-100">
                            ...
                            <th class="px-4 py-4 text-left whitespace-nowrap">Nama Karir</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">Tipe Kepribadian</th>
                        ...
                        @forelse ($karir as $index => $item)
                            <tr class="text-sm text-gray-700 hover:bg-gray-50 transition">
                                <td class="px-4 py-5 font-bold text-[#2E6C9F] text-center whitespace-nowrap">
                                    {{ $karir->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-5 font-semibold uppercase text-[#2E6C9F] whitespace-nowrap">
                                    {{ $item->nama_karir }}
                                </td>
                                <td class="px-4 py-5 whitespace-nowrap text-center">
                                    <span
                                        ...
                                        {{ $item->kepribadian->nama_kepribadian ?? 'N/A' }}
                                    </span>
                                </td>
                                <td class="px-4 py-5 text-gray-500 text-xs leading-relaxed deskripsi-cell">
                                    {{ Str::limit($item->deskripsi_karir, 50) }}
                                </td>
                                <td class="px-4 py-5 text-center whitespace-nowrap">
                                    <div class="flex justify-center gap-2 action-buttons">
                                        <a href="{{ route('admin.karir.show', $item->id_karir) }}"
                                            class="bg-gray-500 text-white px-3 py-1.5 rounded-md text-xs font-bold hover:bg-gray-600 transition shadow-sm">
                                            Detail
                                        </a>
                                        <a href="{{ route('admin.karir.edit', $item->id_karir) }}"
                                            class="bg-[#3D78A8] text-white px-3 py-1.5 rounded-md text-xs font-bold hover:bg-blue-600 transition shadow-sm">
                                            Edit
                                        </a>
                                        <form id="delete-form-{{ $item->id_karir }}"
                                            action="{{ route('admin.karir.destroy', $item->id_karir) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="confirmDelete('{{ $item->id_karir }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-4 py-10 text-center text-gray-400 font-medium">
                                    @if (request('search'))
                                        ...
                                        <p class="font-medium text-lg">Karir tidak ditemukan.</p>
                                    @else
        ...
        <div class="mt-6 pagination-wrapper">
            {{ $karir->appends(request()->query())->onEachSide(1)->links() }}
        </div>
    </div>
    ...
    <script>
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                html: 'Apakah Anda yakin ingin menghapus karir ini? <br><small class="text-red-500 font-medium">Data yang dihapus tidak dapat dipulihkan.</small>',
                icon: 'warning',
