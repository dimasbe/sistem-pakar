...
@section('page-title', 'Edit Aturan')
...
@section('content')
    <div class="max-w-full">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
            ...
            <form id="editAturanForm" action="{{ route('admin.aturan.update', $basisAturan->id_aturan) }}" method="POST"
                novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label
                                class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase tracking-wide">Pertanyaan</label>
                            <div class="relative">
                                <select name="id_pertanyaan" id="id_pertanyaan"
                                    ...
                                    @foreach ($pertanyaan as $p)
                                        <option value="{{ $p->id_pertanyaan }}"
                                            {{ old('id_pertanyaan', $basisAturan->id_pertanyaan) == $p->id_pertanyaan ? 'selected' : '' }}>
                        ...
                        {{-- Pilih Kepribadian --}}
                        <div>
                            <label class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase tracking-wide">Tipe
                                Kepribadian</label>
                            <div class="relative">
                                <select name="id_kepribadian" id="id_kepribadian"
                                    ...
                                    @foreach ($kepribadian as $k)
                                        <option value="{{ $k->id_kepribadian }}"
                                            {{ old('id_kepribadian', $basisAturan->id_kepribadian) == $k->id_kepribadian ? 'selected' : '' }}>
                    ...
                    {{-- Nilai CF Pakar (Input Teks tanpa Spinner) --}}
                    <div>
                        <label class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase tracking-wide">Nilai CF
                            Pakar</label>
                        <input type="text" name="cf_pakar" id="cf_pakar"
                            value="{{ old('cf_pakar', $basisAturan->cf_pakar) }}" placeholder="Contoh: 0.8"
                            class="w-full md:w-1/3 px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-[#2E6C9F] focus:outline-none">
                    </div>
    ...
    <script>
        document.getElementById('editAturanForm').addEventListener('submit', function(e) {
            let errors = [];
            ...
            if (cf === "") {
                ...
                // Konversi koma ke titik jika user input menggunakan koma
