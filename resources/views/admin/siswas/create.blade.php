...
@section('content')
    <div class="max-w-full">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
            {{-- Alert Container untuk Validasi --}}
            ...
            <form id="siswaForm" action="{{ route('admin.siswas.store') }}" method="POST" novalidate>
                @csrf
                <div class="grid grid-cols-1 gap-6">
                    {{-- Nama Lengkap --}}
                    <div>
                        <label class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase">Nama Lengkap</label>
                        <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap') }}"
                            class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-[#2E6C9F] focus:outline-none @error('nama_lengkap') border-red-500 @enderror"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {{-- Email --}}
                        <div>
                            <label class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase">E-mail</label>
                        ...
                        {{-- Kelas Dropdown --}}
                        <div>
                            <label class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase">Kelas</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {{-- Jenis Kelamin --}}
                        <div>
                            <label class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase">Jenis Kelamin</label>
                            <select name="jenis_kelamin"
                                class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-[#2E6C9F] focus:outline-none">
                                <option value="L" {{ old('jenis_kelamin') == 'L' ? 'selected' : '' }}>Laki-laki (L)
                                </option>
                        ...
                        {{-- Password --}}
                        <div>
                            <label class="block text-sm font-bold text-[#2E6C9F] mb-2 uppercase">Password</label>
                            <div class="relative">
                                <input type="password" name="password" id="password"
                                    class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-[#2E6C9F] focus:outline-none"
                                    placeholder="Minimal 6 karakter">
                                <button type="button" onclick="togglePassword('password', 'passwordIcon')"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#2E6C9F]">
