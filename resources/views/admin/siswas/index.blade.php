...
@section('content')
    ...
    <div class="flex justify-between items-center mb-4 header-action-row">
        <h2 class="text-[#2E6C9F] font-bold text-xl uppercase tracking-wider">Manajemen Siswa</h2>
        <a href="{{ route('admin.siswas.create') }}"
    ...
    <div class="mb-4 search-wrapper flex items-center gap-2">
        <form method="GET" action="{{ route('admin.siswas.index') }}" id="searchForm" class="w-full max-w-sm">
            <div class="relative">
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="text-[#2E6C9F] text-sm font-bold border-b border-gray-100">
                            ...
                            <th class="px-4 py-4 text-left whitespace-nowrap">Nama Siswa</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">E-mail</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">Kelas</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">Kelamin</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse ($siswa as $index => $item)
                            <tr class="text-sm text-[#2E6C9F] hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-5 font-bold whitespace-nowrap">
                                    {{ $item->nama_lengkap }}
                                    </>
                                <td class="px-4 py-5 text-center email-cell">
                                    {{ $item->email }}
                                    </>
                                <td class="px-4 py-5 text-center font-bold center-cell">
                                    {{ $item->kelas }}
                                    </>
                                <td class="px-4 py-5 text-center font-bold center-cell">
                                    {{ $item->jenis_kelamin }}
                                ...
                                <td class="px-4 py-5 text-center whitespace-nowrap">
                                    <div class="flex justify-center gap-2 action-buttons">
                                        <a href="{{ route('admin.siswas.edit', $item->id_siswa) }}"
                                        ...
                                        <form id="delete-form-{{ $item->id_siswa }}"
                                            action="{{ route('admin.siswas.destroy', $item->id_siswa) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                onclick="confirmDelete('{{ $item->id_siswa }}', '{{ $item->nama_lengkap }}')"
                                ...
                                @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-gray-400 font-medium">
                                    @if (request('search'))
                                        <i class="fas fa-search text-4xl mb-4 block opacity-30"></i>
                                        <p class="font-medium text-lg">Siswa tidak ditemukan.</p>
                                    @else
    ...
    <script>
        ...
        function confirmDelete(id, name) {
            Swal.fire({
                ...
                html: `Apakah Anda yakin ingin menghapus siswa <br><b class="text-[#2E6C9F]">${name}</b>? <br><small class="text-red-500 font-medium">Data yang dihapus tidak dapat dipulihkan.</small>`,
                icon: 'warning',
