...
@section('title', 'Histori Tes Siswa')
...
@section('content')
    ...
    {{-- Alert Notifikasi - Diselaraskan dengan halaman lainnya --}}
    ...
    <div class="flex justify-between items-center mb-6 header-action-row">
        <div>
            <h2 class="text-[#2E6C9F] font-bold text-xl uppercase tracking-wider">Manajemen Hasil Tes</h2>
            <div class="text-sm text-gray-500 info-stats">Jumlah Riwayat Tes Siswa : <span
                    class="font-bold text-[#2E6C9F]">{{ $tes->total() }}</span></div>
        </div>
        <a href="{{ route('admin.tes.printAll') }}" target="_blank"
            ...
            <i class="fas fa-print mr-2"></i> Cetak Keseluruhan
        </a>
    </div>

    {{-- Search Bar --}}
    <div class="mb-4 search-wrapper flex items-center gap-2">
        <form method="GET" action="{{ route('admin.tes.index') }}" id="searchForm" class="w-full max-w-sm">
            <div class="relative">
                <span class="absolute inset-y-0 left-3 flex items-center text-gray-400 pointer-events-none">
                    <i class="fas fa-search text-xs"></i>
                </span>
                <input type="text" name="search" id="searchInput" placeholder="Cari nama siswa..."
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        ...
        <div class="table-wrapper">
            <div class="overflow-x-auto" style="-webkit-overflow-scrolling: touch;">
                <table class="min-w-full">
                    <thead>
                        <tr class="text-[#2E6C9F] text-sm font-bold border-b border-gray-100 uppercase tracking-tighter">
                            <th class="px-4 py-4 text-center w-16 whitespace-nowrap">No</th>
                            <th class="px-4 py-4 text-left whitespace-nowrap">Nama Lengkap</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">Jenis Kelamin</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">Kelas</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">Hasil Kepribadian</th>
                            <th class="px-4 py-4 text-center whitespace-nowrap">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @forelse ($tes as $index => $item)
                            <tr class="text-sm text-gray-700 hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-5 font-bold text-[#2E6C9F] uppercase whitespace-nowrap">
                                    {{ $item->siswa->nama_lengkap ?? 'Siswa Terhapus' }}
                                </td>
                                <td class="px-4 py-5 text-center font-bold whitespace-nowrap">
                                    @php $jk = strtoupper(substr($item->siswa->jenis_kelamin ?? '-', 0, 1)); @endphp
                                    @if ($jk == 'L')
                                        <span class="text-blue-600 bg-blue-50 px-2 py-1 rounded shadow-sm"
                                            title="Laki-laki">L</span>
                                    @elseif($jk == 'P')
                                        <span class="text-pink-600 bg-pink-50 px-2 py-1 rounded shadow-sm"
                                            title="Perempuan">P</span>
                                    @else
                                        <span class="text-gray-400">-</span>
                                    @endif
                                </td>
                                <td class="px-4 py-5 text-center font-semibold text-gray-600 whitespace-nowrap">
                                    {{ $item->siswa->kelas ?? '-' }}
                                </td>
                                <td class="px-4 py-5 text-center whitespace-nowrap">
                                    @php
                                        // Cek apakah tes teranalisis atau tidak
                                        $isAnalyzed = $item->nilai_cf_total > 0 && $item->id_kepribadian;

                                        if ($isAnalyzed) {
                                            $kombinasi = $hasilKombinasi[$item->id_tes] ?? collect();
                                            $kodeKombinasi = $kombinasi->pluck('kode_kepribadian')->implode(' ');
                                        }
                                    @endphp

                                    @if ($isAnalyzed && !empty($kodeKombinasi))
                                        <span
                                            class="badge-kepribadian bg-blue-50 text-[#2E6C9F] px-3 py-1.5 rounded-full text-[10px] font-bold uppercase border border-blue-100 tracking-widest">
                                            {{ $kodeKombinasi }}
                                        </span>
                                    @else
                                        <span class="text-red-500 text-xs font-bold">Tidak Teranalisis</span>
                                    @endif
                                </td>
                                <td class="px-4 py-5 text-center whitespace-nowrap">
                                    <div class="flex justify-center gap-2 action-buttons">
                                        ...
                                        <form id="delete-form-{{ $item->id_tes }}"
                            ...
                            <tr>
                                <td colspan="6" class="px-4 py-16 text-center text-gray-400">
                                    @if (request('search'))
                                        ...
                                        <p class="font-medium text-lg">Hasil tes tidak ditemukan.</p>
                                        <p class="text-xs mt-1">Coba dengan kata kunci lain</p>
                                    @else
                                        <i class="fas fa-history text-4xl mb-4 block"></i>
                                        <p class="font-medium text-lg">Belum ada riwayat tes siswa.</p>
                                    @endif
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="mt-6 pagination-wrapper">
            {{ $tes->appends(request()->query())->links() }}
        </div>
    </div>
    ...
    <script>
        ...
        function confirmDelete(id, name) {
            Swal.fire({
                ...
                html: `Apakah Anda yakin ingin menghapus riwayat tes <br><b class="text-[#2E6C9F]">${name}</b>? <br><small class="text-red-500 block font-medium">Data yang dihapus tidak dapat dipulihkan.</small>`,
                icon: 'warning',
